...
def create_instances_from_document(
  ...
  while i < len(document):
    ...
    if i == len(document) - 1 or current_length >= target_seq_length:  #  若i是最后一句  或者 current_length现有句子长度 大于 目标句子长度
      if current_chunk:
...
def create_masked_lm_predictions(tokens, masked_lm_prob,
  ...
  num_to_predict = min(max_predictions_per_seq,
                       max(1, int(round(len(tokens) * masked_lm_prob))))   # min(20,句长*15%)  20是上限  masked_lm_prob=15%  max_predictions_per_seq=20
...
def main(input_file,output_file,vocab_file):
...
if __name__ == "__main__":
  ...
  output_file = "./sample_out_text.txt"
  vocab_file = "./vocab.txt"
  main(input_file,output_file,vocab_file)
